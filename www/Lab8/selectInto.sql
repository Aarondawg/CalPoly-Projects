...
BEGIN 
   ...
   FROM customers C
   WHERE  cno = 1111;
   dbms_output.put_line('***Ex1: The customer name is ' ||  cname_);
   dbms_output.put_line('***Ex1: The customer zipcode is ' ||  zip_);

   -- Example 2: use INTO to fetch a value returned by a group 
   --            function
   SELECT count(*) INTO customerCount_
   FROM customers;
   dbms_output.put_line('***Ex2: There are currently ' || 
   ...
   FROM customers C
   WHERE  cno = cno_;
   dbms_output.put_line('***Ex3: The customer name is ' ||  
   ...
   dbms_output.put_line('***Ex4: The customer lives in ' 
        ||  zipcodes_row.zip || ', ' || zipcodes_row.city);

   -- Example 5: illustrates a BAD use of SELECT INTO
   dbms_output.put_line('***Ex5: The customers are: ****');
   SELECT * INTO customer_row
   FROM customers;
