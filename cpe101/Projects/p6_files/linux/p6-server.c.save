...
{
 int i, index, compare, count;
 char user[STR_LEN];
 

  compare = 0;

 read_string(connection, user, STR_LEN);
 ...
 count = users[index].num_msgs;

 for(i = 0; i < count; i++){
   write_string(connection, users[index].messages[i].from);
   write_string(connection, users[index].messages[i].date);
   write_string(connection, users[index].messages[i].message);
 }
...
void handle_delete(CONNECTION_ID connection, struct user users[], int num_users)
{
 ...
 char user[STR_LEN];
 char message_index[STR_LEN];
 int i, msg_index;
 int index, compare, count;

 read_string(connection, user, STR_LEN);
 read_string(connection, message_index, STR_LEN);
 msg_index = atoi(message_index); 
  ...
  if(compare == 0){
     ...
     if(msg_index >= users[index].num_msgs){
	write_string(connection, INVALID_INDEX_MSG);
	  return;
     }
  ...
  if(compare == 1){
     for(i = msg_index; i < count; i++){	
	users[index].messages[i] = users[index].messages[i + 1];
      }
   users[index].num_msgs = users[index].num_msgs - 1;
  }
}
