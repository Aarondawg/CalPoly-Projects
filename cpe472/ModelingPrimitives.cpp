 ...
 *
 *  This code builds a simple 3-box computer.  Modify the
 *  coordinates and colors of each box to match the PC
 *  in front of you.  The model will stay pretty crude, but
 *  match the shapes as closely as you can.  Although you 
 *  can give only one color for the cube, you can specify
 *  one color for each face for the other two primitives.
 *  If the color of your actual machine is too dull, feel free to 
 *  spice it up a bit, the colors they should have made it.
 *
 *  To compile and run this program under Linux, use the following command 
 *  (or put it in a makefile):
 * 
 *  g++ ModelingPrimitives.cpp -DGL_GLEXT_PROTOTYPES -lGL -lglut -lGLU -lm
 *
 *  Note that one of the faces of the keyboard is not specified
 *  correctly.  Be sure to fix it.
 *
 *  Here are some things you should be sure to understand when 
 *  you're done:
 *
 *  1. Why is the CPU and monitor each a single color, but
 *     the keyboard has multiple colors?  Try changing the
 *     monitor to multiple colors.
 *
 *  2. What happens when you scale the Cube primitive?  What
 *     happens when you scale the GL_QUADS primitive?  Why do
 *     you suppose there's a difference?  (We'll get to it in 
 *     class soon.)
 *
 *  3. Identify the data structures used to hold the polygon
 *     information.  Make sure you understand exactly what
 *     each one holds.
 *
 *  4. In what ways can you manipulate the camera with the keyboard
 *     and mouse?  Where is this code?  When we study transforms
 *     later on you'll see how the code works.
 *     
 */
...
void drawCPU() {
   ...
   // set the material
   GLfloat diffuseColor[] = {1.0, 0.8, 0.8, 1.0};
...
void drawKeyboard() {
   ...
   glBegin(GL_QUADS);
      ...
      for (i=0, k=0; k<6; k++, i+=12) {
         ...
         glMaterialfv(GL_FRONT, GL_DIFFUSE, &diffuseColors[k*3]);
...
void drawMonitor() {
   ...
   // vertices for monitor - just name them once
   GLfloat monitorVerts [] = {
      // first four coordinates are the front corners, clockwise from lower left
   ...
   glBegin(GL_QUADS);
      ...
      // k is number of quadrilaterals: 6
      // i is number of vertices per quad: 4
