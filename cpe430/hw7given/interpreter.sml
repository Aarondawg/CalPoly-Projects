...
fun createClosureValue params body env =
   let
       val table = (new_map ())
       val constr = (new_map ())
       val dummy = insert constr "proto_chain" object_prototype
       val buff = insert table "proto_chain" function_prototype
       val constrObj = (Object{table = constr, rf=ref ()})
       val clos = Closure_Value {params=params, body=body, env=env, rf=ref (), props = table}
   in
       (insert constr "constructor" clos; insert table "prototype" constrObj; clos)
   end
...
fun getGlobalHash (Object {table,rf}) = 
    table
  | getGlobalHash v = raise MalformedEnvironment
;
...
and evalExpression (EXP_ID s) env =
       ...
       |  (EXP_DOT {lft,id}) =>
              ...
              in
                  (case obj of
                    ...
                    (*
                        let
                            val proto = lookup props id handle UndefinedIdentifier => Undefined_Value
                        in
                            case proto of
                            (Object _) => (insert (getGlobalHash proto) id rhs; Bool_Value true)
                            | _ => (insert props id rhs; Bool_Value true)
                        end
                        *)
      ...
      (EXP_DOT{lft,id}) =>
        ...
        in
            (case obj of
              (Object{table,rf}) => 
                let
                    val f  = lookup table id handle UndefinedIdentifier => Undefined_Value
                in
                    (case f of
                        Closure_Value closure =>
                           let
                              ...
                              val buff = (insertCurrent newenv "this" obj)
                              val (stmts, funcs) =
  ...
  | evalExpression (EXP_OBJ {props}) env =
    let
        val table = (new_map ())
        val dummy = insert table "proto_chain" object_prototype
        val obj = evalObject props table env
    in
      ...
      (case evalExpression ctorExp env of
        Closure_Value closure =>
           let
              ...
              val proto = getProto (#props closure)
              val y = insert objTable "proto_chain" proto
...
and evalObject [] tbl env = tbl
  | evalObject (x::xs) tbl env = 
    (case x of
        {id,exp} => (insert tbl id (evalExpression exp env);(evalObject xs tbl env))
    )
...
fun createEnvironment () = createEnv [(getGlobalHash global_object)] NONE;
fun initObjects () = 
  (insert (getGlobalHash object_prototype) "proto_chain" Undefined_Value; insert (getGlobalHash function_prototype) "proto_chain" object_prototype; 
    insert (getGlobalHash global_object) "proto_chain" object_prototype)
;
fun evalProgram (PROGRAM {elems}) =
   let
      val (stmts, funcs) = splitSourceElements elems
      val dummy = initObjects ()
   in
